...
export class CourseLeadsRepository {
  ...
  async findMany(query: PaginationQueryDto): Promise<PaginatedCourseLeads> {
    const { page = 1, limit = 20, search } = query;

    const where: Prisma.CourseLeadWhereInput | undefined = search
      ? {
          OR: [
  ...
  findById(id: string): Promise<CourseLead | null> {
    return this.prisma.courseLead.findUnique({ where: { id } });
  }

  create(data: Prisma.CourseLeadCreateInput): Promise<CourseLead> {
    return this.prisma.courseLead.create({ data });
  }
